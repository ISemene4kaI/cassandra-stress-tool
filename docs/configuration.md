...
| `APP_HISTORICAL_READ_ENABLED` | `false` | Enables historical-read-shaped traffic. Without a known id list, reads are still random UUID probes labeled `random_miss_probe`. |
| `APP_HISTORICAL_BUCKETS` | `APP_BUCKETS` | Bucket range for `random_miss_probe`. |
| `APP_RECONNECT_AFTER_CONSECUTIVE_ERRORS` | `10` | Reset the Cassandra session after this many consecutive read/write operation errors. `0` disables operation-error session reset. |
| `APP_READY_MAX_AGE_SECONDS` | `30` | `/readyz` freshness window for `miniapp_last_success_timestamp`. Valid range: `1..3600`. |
...
For ZDM rehearsal, keep `APP_CREATE_SCHEMA=false` and apply schema outside the app on both origin and target.
...
The current app does not load a list of existing historical IDs. `APP_HISTORICAL_READ_ENABLED=true` only changes the bucket range for `random_miss_probe`; it is not a historical data migration validator.

`random_miss_probe` empty reads are expected and do not prove data migration success.
